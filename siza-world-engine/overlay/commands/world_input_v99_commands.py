...
from services.faction_engine import get_faction_registry, membership_authority, upsert_faction, upsert_membership
...
V099_VALIDATION_BUILD = "0.99.0-upchain-authority-relative-fact-sharing"
TEST_FACTION_ID = "TEST-V099-DARSENA-UPCHAIN"
TEST_POLICY_ID = "POLICY-V099-UPCHAIN-001"
TEST_FACT_ID = "FACT-V099-UPCHAIN-001"
TEST_KNOWLEDGE_KEY = "V099_UPCHAIN_REPORT"
...
def _policy(authority_relation="HIGHER_THAN_SOURCE"):
    return {
        "id": TEST_POLICY_ID,
        "enabled": True,
...
def _faction(authority_relation="HIGHER_THAN_SOURCE"):
    return {
        "id": TEST_FACTION_ID,
        "name": TEST_FACTION_ID,
        "active": True,
        "canon_status": "prototype",
        "fact_share_policies": [_policy(authority_relation)],
    }
...
def _seed_fact(npc, site):
    upsert_knowledge_fact(
        npc,
        {
            ...
            "severity": 5,
            "topic": "reporte ascendente de prueba",
            "text": "Un incidente de prueba debe subir por una cadena de mando estrictamente ascendente.",
            "canon_status": "prototype",
            "source": {
                "kind": "VALIDATOR_SEED",
...
def _managed_rules(npc):
    return [
        ...
        if str((row or {}).get("authored_rule_id") or "") == TEST_POLICY_ID
    ]
...
class CmdSizaValidateV99(Command):
    key = "siza-validate-v99"
    aliases = ["validate-v99"]
    ...
    def func(self):
        ...
        if not informant or not mara or not worker or not site or not calle or not plaza or registry is None:
            self.caller.msg("[V0.99 VALIDATION] FAIL | persistent context missing")
            return
        ...
        self.caller.msg(f"=== SIZA VALIDATION v0.99 | {V099_VALIDATION_BUILD} ===")
        self.caller.msg(
            "institutional HIGHER_THAN_SOURCE policy -> exact Fact climbs only to strictly higher current faction authority -> equal peers do not relay -> top authority stops naturally"
        )
        ...
        try:
            ...
            upsert_membership(worker, {"faction_id": TEST_FACTION_ID, "active": True, "role": "officer", "authority_level": 500})

            first_sync = sync_faction_fact_share_policies(informant)
            first_rules = _managed_rules(informant)
            check(
                "inherited-upchain-policy-carries-additive-authority-relation-capability-with-historical-builds-stable",
                len(first_rules) == 1
                and str(first_rules[0].get("authority_relation") or "") == "HIGHER_THAN_SOURCE"
                ...
                and FACT_SHARE_NEED_AWARE_SELECTION_BUILD == "0.95.0-need-aware-nearest-fact-share-selection"
                and FACT_SHARE_AUTHORITY_RELATION_BUILD == V099_VALIDATION_BUILD,
                f"sync={first_sync.get('status')} relation_build={FACT_SHARE_AUTHORITY_RELATION_BUILD}",
            )

            first_refresh = refresh_fact_share_obligations(informant)
            check(
                "authority-100-source-selects-nearest-strictly-higher-recipient-through-existing-nearest-routing",
                _candidate_ids(informant) == {mara_id}
                and membership_authority(informant, TEST_FACTION_ID) == 100
                and membership_authority(mara, TEST_FACTION_ID) == 500
                and any(
                    str(row.get("target_npc_id") or "") == mara_id
                    and str(row.get("authority_relation") or "") == "HIGHER_THAN_SOURCE"
            ...
            check(
                "first-upchain-hop-uses-existing-decision-movement-contact-and-exact-local-fact-transfer",
                first_step.get("status") == "GOAL_COMPLETED"
                and informant.location == mara.location
                and mara_fact is not None
                and len(first_history) == 1
                and str((first_history[-1] or {}).get("source_npc_id") or "") == informant_id
                and str((first_history[-1] or {}).get("target_npc_id") or "") == mara_id
                and str((first_history[-1] or {}).get("mode") or "") == "DIRECT_LOCAL",
                f"status={first_step.get('status')} history={first_history}",
            )

            sync_faction_fact_share_policies(mara)
            equal_refresh = refresh_fact_share_obligations(mara)
            check(
                "equal-authority-peer-is-not-a-valid-upchain-recipient-after-first-hop",
                membership_authority(mara, TEST_FACTION_ID) == 500
                and membership_authority(worker, TEST_FACTION_ID) == 500
                and not _candidate_ids(mara)
                and not list(equal_refresh.get("materialized") or []),
                f"mara_auth={membership_authority(mara, TEST_FACTION_ID)} worker_auth={membership_authority(worker, TEST_FACTION_ID)} candidates={sorted(_candidate_ids(mara))}",
            )

            upsert_membership(worker, {"faction_id": TEST_FACTION_ID, "active": True, "role": "officer", "authority_level": 800})
            promoted = refresh_fact_share_obligations(mara)
            worker_obligation = _find_obligation(mara, worker_id)
            check(
                "promotion-above-source-authority-dynamically-opens-the-next-upchain-branch",
                membership_authority(worker, TEST_FACTION_ID) == 800
                and _candidate_ids(mara) == {worker_id}
                and worker_obligation is not None
                and worker_obligation.get("active") is True
                and any(str(row.get("obligation_id") or "") == worker_oid for row in list(promoted.get("materialized") or [])),
                f"candidates={sorted(_candidate_ids(mara))} obligation={worker_obligation}",
            )

            upsert_faction(_faction("SIDEWAYS"))
            bad_sync = sync_faction_fact_share_policies(mara)
            bad_refresh = refresh_fact_share_obligations(mara)
            cancelled_worker = _find_obligation(mara, worker_id)
            check(
                "malformed-authority-relation-fails-closed-and-cancels-existing-pending-upchain-branch",
                any(row.get("reason") == "BAD_AUTHORITY_RELATION" for row in list(bad_refresh.get("skipped") or []))
                and cancelled_worker is not None
                and cancelled_worker.get("active") is False
                and cancelled_worker.get("status") == "cancelled"
                and cancelled_worker.get("cancellation_reason") == "BAD_AUTHORITY_RELATION",
                f"sync={bad_sync.get('status')} obligation={cancelled_worker}",
            )

            upsert_faction(_faction())
            repair_sync = sync_faction_fact_share_policies(mara)
            repaired = refresh_fact_share_obligations(mara)
            repaired_obligation = _find_obligation(mara, worker_id)
            check(
                "correcting-authority-relation-reactivates-the-same-worker-obligation-id-without-duplication",
                repaired_obligation is not None
                and repaired_obligation.get("active") is True
                and repaired_obligation.get("status") == "pending"
                and any(
                    str(row.get("obligation_id") or "") == worker_oid and row.get("created") is False
                    for row in list(repaired.get("materialized") or [])
                ),
                f"sync={repair_sync.get('status')} obligation={repaired_obligation}",
            )
            ...
            check(
                "second-upchain-hop-preserves-ordered-two-hop-transfer-history-through-existing-local-authority",
                second_step.get("status") == "GOAL_COMPLETED"
                and mara.location == worker.location
                ...
                and all(str((row or {}).get("mode") or "") == "DIRECT_LOCAL" for row in history),
                f"status={second_step.get('status')} history={history}",
            )
        ...
        self.caller.msg("")
        self.caller.msg(
            "PERSISTENT SYSTEM RETAINED: v0.89-v0.98 social/institutional authority remains unchanged; v0.99 only adds optional strictly-upchain faction recipient eligibility before existing selection/contact/transfer"
        )
        self.caller.msg("")
        self.caller.msg("========================================================")
