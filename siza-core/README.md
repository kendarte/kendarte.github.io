...
- `creature-rules.js` owns creature counters, equipped-card lookup, descriptor-driven Equipment power bonuses, effective power/toughness, aligned battlefield add/remove mutations, the pure combat outcome plan, and the pure AI blocker assignment plan. Arena applies combat results and blocker assignments to match state while life, graveyards, logs, win state, turn state, and UI orchestration remain outside the shared core.
