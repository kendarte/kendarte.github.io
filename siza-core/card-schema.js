...
const CARD_SCHEMA_VERSION='1.7.0';
...
function normalizeCard(input={}){
 ...
 const card={
  ...
  frameUrl:text(input.frameUrl,''),frameAssetKey:text(input.frameAssetKey,''),cardType:type,type,subtype:text(input.subtype,''),affinity:text(input.affinity,'multi'),difficulty:Math.max(0,int(input.difficulty,0)),equipCost:Math.max(0,int(input.equipCost,0)),
...
function validateCard(input={}){
 ...
 if(card.cardType!=='Land'&&card.difficulty<=0)errors.push('Una carta no-Land necesita Manafestación mayor que 0.');if(card.cardType==='Creature'&&(card.power==null||card.toughness==null))errors.push('Una Creature necesita Ataque y Defensa.');if(card.cardType!=='Creature'&&(card.power!=null||card.toughness!=null))warnings.push('Ataque/Defensa sólo se muestran en Creature.');
...
function cardToMobileShape(input={}){
 const c=normalizeCard(input),out={id:c.id,name:c.name,template:c.template,frameUrl:c.frameUrl,type:c.cardType,difficulty:c.difficulty,pips:clone(c.pips),text:c.rules,flavor:c.flavor,art:c.art,glyph:c.glyph,artUrl:c.artUrl,artTransform:clone(c.artTransform),battleSpriteUrl:c.battleSpriteUrl,battleSpriteTransform:clone(c.battleSpriteTransform),effects:clone(c.effects)};
