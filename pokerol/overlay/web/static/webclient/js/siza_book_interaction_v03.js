(function () {
    ...
    var ROOM_PRESENTATION = {
        "Embarcadero de Campana": {name:"Bell Dock",description:"A working dock platform where cargo, crews, and arrivals converge under the noise of the harbor."},
        "Patio de Mineral": {name:"Mineral Yard",description:"A rough handling yard where wet mineral loads are sorted before moving deeper into the dock district."},
        "Plaza de Recepcion": {name:"Receiving Square",description:"A busy receiving square linking dock traffic, local services, and the lower work streets."},
        "Calle de Servicio": {name:"Service Lane",description:"A narrow work lane used by dock crews and supply traffic; cargo noise carries between the adjoining businesses."},
        "Casa de Remedio": {name:"Remedy House",description:"A modest treatment house serving dock workers and families from the surrounding district."},
        "Cantina de Turno": {name:"Shift Canteen",description:"A cramped canteen where crews eat, trade news, and wait for the next change of shift."},
        "Pescaderia de Darsena": {name:"Dockside Fishery",description:"A cramped dockside shop with a scarred work counter, brine in the air, and storage pushed toward the back."},
        "Trastienda de la Pescaderia": {name:"Fishery Back Room",description:"A small storage room behind the shop, crowded with supplies and the residue of daily dock work."}
    };
    ...
    function entityLabel(raw){var name=rawEntityName(raw);return ENTITY_LABELS[name]||name;}
    function exitLabel(raw){return EXIT_LABELS[key(raw)]||clean(raw);}
    ...
    function translateMemory(text){
        ...
        for(var i=0;i<MEMORY_TRANSLATIONS.length;i+=1){if(value.indexOf(MEMORY_TRANSLATIONS[i][0])===0)return MEMORY_TRANSLATIONS[i][1];}
        return value;
    }
    ...
    function localizeMemories(){
        ...
        if(!list)return;
        Array.prototype.forEach.call(list.children,function(node){var translated=translateMemory(node.textContent);if(node.textContent!==translated)node.textContent=translated;});
        var count=list.children.length,next=count===1?"1 memory":count+" memories";
        if(summary&&summary.textContent!==next)summary.textContent=next;
    ...
    function localizeRoom(){
        ...
        if(!location)return;
        var visibleName=clean(location.textContent),rawName=location.getAttribute("data-raw-room")||visibleName;
        if(ROOM_PRESENTATION[visibleName]){rawName=visibleName;location.setAttribute("data-raw-room",rawName);}
        ...
        if(presentation){
            ...
            if(placeholder&&placeholder.textContent!==presentation.name)placeholder.textContent=presentation.name;
        }else if(placeholder&&placeholder.textContent!==visibleName){placeholder.textContent=visibleName||"Location";}
    }
    ...
    function replaceKnownUiText(node){
        ...
        var exact={
            "EXPLORACIÓN":"EXPLORATION","DIÁLOGO":"DIALOGUE","COMBATE":"COMBAT","Conectado":"Connected","Conectando…":"Connecting…","Desconectado":"Disconnected",
            "Error de conexión":"Connection error","ENVIAR":"SEND","ESPERANDO":"WAITING","¿Qué haces?":"What do you do?","Sin personaje":"No character",
            ...
            "No entiendo esa acción todavía.":"I can't resolve that action yet."
        };
        if(exact[text]&&node.textContent!==exact[text])node.textContent=exact[text];
            ...
            var memory=translateMemory(text);
            if(memory!==text)node.textContent=memory;
        }

        var current=node.textContent||"";
        Object.keys(ROOM_PRESENTATION).forEach(function(rawName){if(current.indexOf(rawName)!==-1)current=current.split(rawName).join(ROOM_PRESENTATION[rawName].name);});
        if(node.textContent!==current)node.textContent=current;
    }

    function localizeDynamicUi(){
        ["siza-mode-label","siza-connection-label","siza-current-prompt","siza-inputsend","siza-context-label","siza-focus-portrait-name"].forEach(function(id){replaceKnownUiText(byId(id));});
    ...
    function requestStats(){
        ...
        if(!window.Evennia||!Evennia.isConnected()){if(status)status.textContent="World Engine is not connected.";return;}
    ...
    function init(){
        ...
        observe("siza-location-label",localizeRoom);observe("siza-scene-description",localizeRoom);observe("siza-knowledge-list",localizeMemories);observe("siza-knowledge-summary",localizeMemories);
        observe("siza-mode-label",localizeDynamicUi);observe("siza-connection-label",localizeDynamicUi);observe("siza-current-prompt",localizeDynamicUi);observe("siza-inputsend",localizeDynamicUi);
        observe("siza-context-label",localizeDynamicUi);observe("siza-focus-portrait-name",localizeDynamicUi);observe("siza-messagewindow",localizeDynamicUi);

        if(window.Evennia&&Evennia.emitter)Evennia.emitter.on("siza_character_stats",onStats);
        renderActionSource("siza-exits","siza-exits-actions","exit");renderActionSource("siza-characters","siza-characters-actions","person");renderActionSource("siza-visible","siza-visible-actions","object");
        localizeDynamicUi();
    }

    window.SizaBookInteractionV03=Object.freeze({splitItems:splitItems,renderActionSource:renderActionSource,openPanel:openPanel,requestStats:requestStats,localizeRoom:localizeRoom});
