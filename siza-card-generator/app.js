...
const EXAMPLE={
 id:'memoria_reina_ahogada',name:'Memoria de la Reina Ahogada',template:'standard',frameUrl:'',frameAssetKey:'',cardType:'Creature',subtype:'Avatar',affinity:'azul',difficulty:8,cost:5,pips:{U:3},artId:'queen_drowned',artUrl:'',artAssetKey:'',artTransform:{x:50,y:35,scale:1},rules:'Al materializarse, roba dos cartas y luego descarta una.',flavor:'La corona sobrevivió porque nadie recordó enterrarla.',force:5,resistance:5,setCode:'SZA',cardNumber:'036',glyph:'♛',effects:[{event:'enter',type:'draw',target:'self',amount:2},{event:'enter',type:'discard',target:'self',amount:1,choice:'owner'}]
};
...
function readForm(opts={}){
 ...
 const externalArt=$('artUrl').value.trim(),template=$('templateSelect')?.value||'standard',entry=templateEntry(template),storedFrame=currentFrameInlineUrl||entry?.frameUrl||'',previewFrame=currentFrameObjectUrl||storedFrame;
...
function writeForm(input){
 ...
 revokeArtObjectUrl();currentArtAssetKey=c.artAssetKey||'';revokeFrameObjectUrl();refreshTemplateSelect(c.template||'standard');const entry=templateEntry(c.template);currentFrameAssetKey=c.frameAssetKey||entry?.frameAssetKey||'';currentFrameInlineUrl=c.frameUrl||entry?.frameUrl||'';
...
function newBlank(){return{...EXAMPLE,id:'card_'+Date.now(),name:'Carta sin nombre',template:'standard',frameUrl:'',frameAssetKey:'',difficulty:1,cost:1,pips:{},artId:'',artUrl:'',artAssetKey:'',artTransform:{x:50,y:50,scale:1},rules:'',flavor:'',force:2,resistance:2,cardNumber:'000',glyph:'✦',effects:[]};}
function newCard(){writeForm(newBlank());setMessage('Nueva carta. Puede elegir/subir un template, cargar ilustración y guardar cuando esté lista.');}
...
async function prepareHandoff(){
 ...
 const stored=validation.card;localStorage.setItem(DRAFT_KEY,JSON.stringify(stored));let card=await materializeArt(stored);card=await materializeFrame(card);const payload={action:'test-card',card:SizaCardSchema.cardToMobileShape(card),generatorCard:card,target:'collection',returnUrl,createdAt:new Date().toISOString()};
 try{localStorage.setItem(HANDOFF_KEY,JSON.stringify(payload));location.href=returnUrl;}catch(e){setMessage('El arte o frame es demasiado grande para el handoff temporal. Vuelva a subirlo para que el generador lo optimice.');}
}
...
window.addEventListener('beforeunload',()=>{revokeArtObjectUrl();revokeFrameObjectUrl();});loadInitial();
})();
