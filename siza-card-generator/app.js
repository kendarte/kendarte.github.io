...
function loadInitial(){let draft=null;try{draft=JSON.parse(localStorage.getItem(DRAFT_KEY)||'null')}catch(e){}if(draft)writeForm(draft);else writeForm({...EXAMPLE,id:requestedCardId||EXAMPLE.id});if(requestedCardId)$('handoffStatus').textContent=`Editando ${requestedCardId}. Probar en Mobile Test reemplaza sólo la carta temporal con este mismo ID.`;}
