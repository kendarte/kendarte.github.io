...
(async()=>{
  ...
  {
    const OM=o.createMatch(false,'immediate'),NM=n.createMatch(false,'immediate');for(const M of[OM,NM]){M.player.hand=[];M.enemy.hand=[];M.player.battlefield=[];M.player.powerCounters=[];M.player.summonedOn=[];M.player.artifacts=[];M.player.equipment=[];M.enemy.battlefield=['ignimite'];M.enemy.powerCounters=[0];M.enemy.summonedOn=[0];M.enemy.artifacts=['prism'];M.enemy.equipment=[{id:'tideblade',target:0}];M.stack=[{id:'s1',cardId:'leviathan',owner:'player'}];M.log=[]}o.setMatch(OM);n.setMatch(NM);o.resolveTopStack();n.resolveTopStack();assert(snap(OM)===snap(NM),`player Leviathan mismatch :: ${snap(OM)} / ${snap(NM)}`);compared++;
  }
  {
    const OM=o.createMatch(false,'immediate'),NM=n.createMatch(false,'immediate');for(const M of[OM,NM]){M.player.hand=[];M.enemy.hand=[];M.player.battlefield=['servitor'];M.player.powerCounters=[0];M.player.summonedOn=[0];M.player.artifacts=['prism'];M.player.equipment=[];M.enemy.battlefield=[];M.enemy.powerCounters=[];M.enemy.summonedOn=[];M.enemy.artifacts=[];M.enemy.equipment=[];M.stack=[{id:'s2',cardId:'leviathan',owner:'enemy'}];M.log=[]}o.setMatch(OM);n.setMatch(NM);o.resolveTopStack();n.resolveTopStack();assert(snap(OM)===snap(NM),`enemy Leviathan mismatch :: ${snap(OM)} / ${snap(NM)}`);compared++;
  }
