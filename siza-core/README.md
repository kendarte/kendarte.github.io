...
- `creature-rules.js` owns creature counters, equipped-card lookup, descriptor-driven Equipment power bonuses, effective power/toughness, aligned battlefield add/remove mutations, and a pure combat outcome plan. Arena applies that plan to life, counters, graveyards, logs, win state, turn state, and UI so orchestration remains outside the shared core.
