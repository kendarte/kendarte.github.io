(function(){
  ...
  var BUILD='0.1.1-persistent-assets';
  var waiters=[];
  var bound=false;
  ...
  function onResult(args){
    var packet=packetFrom(args);if(!packet.status)return true;
    waiters.slice().forEach(function(w){
      var matched=false;try{matched=w.test(packet)}catch(e){}
      if(!matched)return;var index=waiters.indexOf(w);if(index>=0)waiters.splice(index,1);clearTimeout(w.timer);
  ...
  function waitFor(test,timeout){
    return new Promise(function(resolve,reject){
      ...
      w.timer=setTimeout(function(){var i=waiters.indexOf(w);if(i>=0)waiters.splice(i,1);reject(new Error('El servidor no confirmó la carga.'))},timeout||15000);waiters.push(w);
    });
  }
  function bindEmitter(){
    if(bound)return true;if(!window.Evennia||!Evennia.emitter||typeof Evennia.emitter.on!=='function')return false;
    Evennia.emitter.on('pokerol_asset_result',onResult);bound=true;return true;
  }
  ...
  function doUpload(meta,file,onProgress){
    ...
    return ensureBound().then(function(){
      ...
      return file.arrayBuffer().then(function(buffer){
        ...
        function next(){
          if(offset>=bytes.length){send('pokerol-asset-finish',{token:token});return waitFor(function(p){return (p.token===token&&p.status==='UPLOAD_DONE')||p.status==='ERROR'},20000)}
          var chunk=bytes.subarray(offset,Math.min(bytes.length,offset+chunkSize)),thisIndex=index;
          send('pokerol-asset-chunk',{token:token,index:thisIndex,data:bytesToBase64(chunk)});
          return waitFor(function(p){return (p.token===token&&p.status==='CHUNK_OK'&&Number(p.index)===thisIndex)||p.status==='ERROR'},15000).then(function(){offset+=chunk.length;index+=1;if(typeof onProgress==='function')onProgress(Math.min(1,offset/bytes.length));return next()});
        }
        return next();
      });
    });
  ...
  function enqueue(task){
    var run=queue.catch(function(){}).then(task);queue=run.catch(function(){});return run;
  }
  function uploadFile(meta,file,onProgress){return enqueue(function(){return doUpload(meta,file,onProgress)})}
  function clearAsset(meta){
    return enqueue(function(){return ensureBound().then(function(){send('pokerol-asset-clear',{kind:meta.kind,dbref:meta.dbref||null,hotspot_id:meta.hotspot_id||''});return waitFor(function(p){return p.status==='ASSET_CLEARED'||p.status==='ERROR'},15000)})});
  }
